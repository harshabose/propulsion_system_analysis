...
public:
    ...
    propulsion_system (operationalPoint_h::operationalPoint *IN_ATMOSPHERE, const bool IN_CONTROL_WITH_RPM, const bool IN_CONTROL_WITH_BLADE_PITCH_ANGLE = false, const float IN_FIXED_RPM = 0.0f, const float IN_FIXED_PITCH = 0.0f) {
        ...
        std::cout << "CREATED AN INSTANCE OF PROPULSION SYSTEM ANALYSIS TOOL..." << std::endl;
    }
    ...
    void update_propulsion_system_design (const size_t IN_NUMBER_OF_PROPULSION_UNITS, const float IN_RADIUS, const size_t IN_NUMBER_OF_BLADES, const float IN_ROOT_CHORD, const std::vector<float>& IN_TAPER_EQUATION, const std::vector<float>& IN_TWIST_EQUATION) {
        ...
        std::cout << "PERFORMING POWER REQUIRED ANALYSIS WITH THE FOLLOWING DESIGN PARAMETERS:" << std::endl;
        std::cout << "NUMBER_OF_PROPULSION_UNITS -> " << IN_NUMBER_OF_PROPULSION_UNITS << std::endl;
        std::cout << "RADIUS -> " << IN_RADIUS << std::endl;
        std::cout << "NUMBER_OF_BLADES -> " << IN_NUMBER_OF_BLADES << std::endl;
        std::cout << "ROOT_CHORD -> " << IN_ROOT_CHORD << std::endl;
        std::cout << "TAPER RATIO -> " << IN_TAPER_EQUATION[0] << std::endl;
        std::cout << "BLADE TWIST -> " << IN_TWIST_EQUATION[0] << std::endl;
    ...
    private:
        ...
        void calculate_propeller_forces_moments(const float guess_RPM, const float guess_Pitch) {
            ...
            try {
                ...
                this->total_power_required = std::numeric_limits<float>::max();
                this->obtained_thrust_coeffecient = std::numeric_limits<float>::max();
                std::cerr << "power declared to max value " << this->total_power_required << ", " << this->obtained_thrust_coeffecient  << std::endl;
            }
        }
